...
| `ENABLE_FUEL_PUMP_SAFETY` | Multi-gate fuel pump FSM (PRIME → ARMED → RUNNING) with on-board coil voltage ADC sampling. Owns a relay (default R1). Broadcasts `IGNITION_DATA (0x310)` + `FUEL_PUMP_STATE (0x311)`. | `FUEL_PUMP_RELAY`, `FUEL_PUMP_PRIME_MS`, `FUEL_PUMP_RPM_THRESHOLD`, `FUEL_PUMP_STALL_MS`, `FUEL_PUMP_DEFAULT_MODE`; for the COIL gate also: `IGN_COIL_ADC_PIN`, `IGN_COIL_DIVIDER_RATIO`, `IGN_COIL_ON_THRESHOLD_CV`, `IGN_COIL_OFF_THRESHOLD_CV` |
