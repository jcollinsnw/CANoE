...
| cardputer        | 0x06    | configs/cardputer.h              | ENABLE_M5_CARDPUTER — M5Stack Cardputer (ESP32-S3); TFT CAN terminal + keyboard CLI; four screens: CAN (raw hex log), FEED (human-readable), COMMANDS (action macros), SETTINGS (BT/WiFi/speaker); ESPNOW_ONLY mode (no SoftAP/web server). |
