...
| cardputer        | 0x06    | configs/cardputer.h              | ENABLE_M5_CARDPUTER — M5Stack Cardputer (ESP32-S3); TFT CAN terminal + keyboard CLI; three screens: CAN (raw hex log), FEED (human-readable), MENU (action macros); ESPNOW_ONLY mode (no SoftAP/web server). |
...
| `ENABLE_M5_CARDPUTER` | mod_m5_cardputer | M5Stack Cardputer TFT display + keyboard CLI; relay status bar (#006DAC). Three screens: **CAN** (raw hex frame log), **FEED** (human-readable decoded frames), **MENU** (action macros). `fn` toggles CAN↔MENU. `opt` enters status bar selection mode: `;`/`.` navigate relays and the CAN/FEED/MENU label; Enter/Space toggles a relay or opens a dropdown to switch screens; Del or Opt exits. Status bar shows relay boxes (green=ON, grey=OFF; in selection mode selected=grey, unselected-OFF=acapulco-blue), battery gauge (dark green/yellow/red), peer count pill (green/red). CLI bar shows status events when CLI is closed. `;`/`.` scroll 8-entry command history in the CLI. Requires M5Cardputer library. |
